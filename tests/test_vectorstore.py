...
from src.rag_agent.agent.state import ChunkMetadata, DocumentChunk
...
class TestDuplicateDetection:
    ...
    def test_new_chunk_is_not_duplicate(
        self, tmp_path, sample_chunk: DocumentChunk, test_settings
    ) -> None:
...
class TestRetrieval:
    ...
    def test_relevant_query_returns_results(
        ...
        store = VectorStoreManager(settings=test_settings)
        store.ingest([sample_chunk])
        results = store.query("LSTM gate mechanism", k=5)
        assert len(results) > 0

    def test_irrelevant_query_returns_empty(self, tmp_path, sample_chunk: DocumentChunk, test_settings) -> None:
        """
        ...
        store = VectorStoreManager(settings=test_settings)
        store.ingest([sample_chunk])
        results = store.query("history of the roman empire", k=5)
        assert results == []
    ...
    def test_topic_filter_restricts_results(
        ...
        results = store.query("neural networks", k=5, topic_filter="LSTM")
    ...
    def test_results_sorted_by_score_descending(
        ...
        # Create additional chunks with varying similarity
        chunk2 = DocumentChunk(
            chunk_id=VectorStoreManager.generate_chunk_id("test2.md", "LSTM networks are great"),
            chunk_text="LSTM networks are great for sequence modeling.",
            metadata=ChunkMetadata(
                topic="LSTM",
                difficulty="intermediate",
                type="concept_explanation",
        ...
        chunk3 = DocumentChunk(
            chunk_id=VectorStoreManager.generate_chunk_id("test3.md", "Backpropagation through time"),
            chunk_text="Backpropagation through time is used in RNNs.",
            metadata=ChunkMetadata(
                topic="RNN",
                difficulty="intermediate",
                type="algorithm",
        ...
        store = VectorStoreManager(settings=test_settings)
        store.ingest([sample_chunk, chunk2, chunk3])
        results = store.query("LSTM gate mechanism", k=5)
