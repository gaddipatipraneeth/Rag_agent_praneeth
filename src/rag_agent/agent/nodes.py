...
def query_rewrite_node(state: AgentState) -> dict:
    ...
    """
    # Extract the latest HumanMessage
    ...
    rewrite_prompt = (
        "Rewrite the following user query to be more effective for vector similarity search "
        "against a deep learning corpus. Focus on key technical terms and concepts. "
        "Keep it concise, under 20 words.DO not provide any explanations, just give the rewritten query\n\n"
...
def generation_node(state: AgentState) -> dict:
    ...
    context_message = HumanMessage(content=f"Context:\n{context}")
    ...
    trimmed_history = trim_messages(
        ...
        strategy="last",
        token_counter=len,  # Simple token count, could be improved
        start_on="human",
    )
    
    human_message = HumanMessage(content=state["original_query"])
    messages = [system_message, context_message] + trimmed_history + [human_message]
    ...
    sources = [chunk.to_citation() for chunk in state["retrieved_chunks"]]
    ...
    return {"final_response": agent_response, "messages": [ai_message]}
...
def should_retry_retrieval(state: AgentState) -> str:
    ...
    # LangGraph may pass state as a dict during edge evaluation,
    # so support both Attr and Mapping access styles.
    no_context = None
    if isinstance(state, dict):
        no_context = state.get("no_context_found", False)
    else:
        no_context = getattr(state, "no_context_found", False)
