...
class VectorStoreManager:
    ...
    def _initialise(self) -> None:
        ...
        logger.info(f"ChromaDB initialised: collection '{self._settings.chroma_collection_name}' with {self._collection.count()} items")
    ...
    def ingest(self, chunks: list[DocumentChunk]) -> IngestionResult:
        ...
        for chunk in chunks:
            ...
            result.ingested += 1
        logger.info(f"Ingested {result.ingested} chunks, skipped {result.skipped}")
        return result
    ...
    def list_documents(self) -> list[dict]:
        ...
        for metadata in result["metadatas"]:
            ...
            if source not in sources:
                sources[source] = {"source": source, "topic": metadata.get("topic", "N/A"), "chunk_count": 0}
    ...
    def get_document_chunks(self, source: str) -> list[DocumentChunk]:
        ...
        result = self._collection.get(where={"source": source}, include=["documents", "metadatas"])
        chunks = []
        for doc, metadata in zip(result["documents"], result["metadatas"]):
            chunk_metadata = ChunkMetadata(**metadata)
            chunk_id = self.generate_chunk_id(source, doc)
            chunk = DocumentChunk(
                chunk_id=chunk_id,
                chunk_text=doc,
                metadata=chunk_metadata
    ...
    def get_collection_stats(self) -> dict:
        ...
        topics = set()
        sources = set()
        bonus_topics = {"RNN", "CNN", "Transformer", "GAN", "Autoencoder"}  # example bonus topics
        bonus_present = False
        for metadata in result["metadatas"]:
            topics.add(metadata.get("topic", "N/A"))
            sources.add(metadata["source"])
            if metadata.get("topic") in bonus_topics:
                bonus_present = True
        return {
    ...
    def delete_document(self, source: str) -> int:
        ...
        result = self._collection.get(where={"source": source}, include=["ids"])
